...
import { Globe, User, ShieldCheck, Monitor, CheckCircle, X, UserPlus, ArrowRight, Lock, Zap } from 'lucide-react';
...
const OnboardingWizard = () => {
    ...
    return (
        <div className={`fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-3xl text-white font-sans ${isModalClosing ? 'animate-pop-out' : 'animate-modal'}`} onClick={closeModal}>
            <div className="w-full max-w-[900px] h-[600px] bg-[#0a0a0c]/80 border border-white/10 rounded-[2rem] shadow-[0_50px_100px_rgba(0,0,0,0.8)] overflow-hidden flex transition-all duration-500" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 relative overflow-hidden bg-[#121214]">
                    ...
                    <div className="absolute inset-y-0 left-0 w-[400%] flex transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)]" style={{ transform: `translateX(-${onboardingStep * 25}%)` }}>

                        {/* Krok 0: Profil Lokalny */}
                        <div className="w-1/4 h-full p-12 flex flex-col justify-center relative">
                            ...
                            <h3 className="text-4xl font-black text-white mb-4 tracking-tight">Welcome to QBrowse.</h3>
                            <p className="text-sm text-white/50 mb-10 leading-relaxed max-w-md">Your gateway to the local-first web. Let's start by creating an isolated local profile. No cloud accounts required.</p>

                            <div className="flex flex-col gap-2 mb-8">
                                <label className="text-[10px] font-bold uppercase tracking-widest text-white/40">Display Name</label>
                                <input type="text" value={obUsername} onChange={e => setObUsername(e.target.value)} placeholder="e.g. Admin" className="w-full max-w-sm bg-black/40 border border-white/10 rounded-2xl px-5 py-4 text-lg text-white outline-none focus:border-accent/50 focus:shadow-[0_0_30px_var(--accent-20)] transition-all" />
                            </div>

                            <button onClick={() => setOnboardingStep(1)} disabled={!obUsername.trim()} className="px-8 py-4 bg-accent text-black rounded-2xl font-bold self-start hover:scale-105 transition-transform shadow-[0_10px_20px_var(--accent-20)] disabled:opacity-50 disabled:hover:scale-100 flex items-center gap-2">
                        ...
                        <div className="w-1/4 h-full p-12 flex flex-col justify-center relative">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <button onClick={() => setOnboardingStep(2)} disabled={obPassword.length < 6} className="px-8 py-4 bg-emerald-500 text-white rounded-2xl font-bold hover:scale-105 transition-transform shadow-[0_10px_20px_rgba(16,185,129,0.3)] disabled:opacity-50 disabled:hover:scale-100 flex items-center gap-2">
                                    Encrypt & Next <ShieldCheck size={16} />
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="w-1/4 h-full p-12 flex flex-col justify-center relative">
                            ...
                            <div className="flex items-center gap-4 mt-auto">
                                ...
                                <button onClick={() => setOnboardingStep(3)} className="px-8 py-4 bg-blue-500 text-white rounded-2xl font-bold hover:scale-105 transition-transform shadow-[0_10px_20px_rgba(59,130,246,0.3)] flex items-center gap-2">
                                    Link & Next <ArrowRight size={16} />
                                </button>
                            </div>
                        </div>

                        {/* Krok 3: Launch */}
                        <div className="w-1/4 h-full p-12 flex flex-col items-center justify-center relative text-center">
                            ...
                            <h3 className="text-4xl font-black text-white mb-4 tracking-tight">You're all set, {obUsername || 'Admin'}.</h3>
                            <p className="text-sm text-white/50 mb-10 leading-relaxed max-w-md">Your local profile is secure. Engine is primed. Welcome to the future of browsing.</p>

                            <button onClick={closeModal} className="px-10 py-5 bg-accent text-black rounded-3xl text-lg font-black tracking-wide hover:scale-110 transition-transform shadow-[0_15px_30px_var(--accent-30)] flex items-center gap-3">
