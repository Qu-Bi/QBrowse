...
import { User, Layers, Ghost, Play, WifiOff, X, Globe } from 'lucide-react';
...
export default function TabMap() {
    ...
    const closeTabMap = useUIStore(state => state.closeTabMap);
    ...
    useEffect(() => {
        ...
        const loop = (time) => {
            ...
            const target = isOrbitHoveredRef.current ? 0 : 1;
            const lerpFactor = isOrbitHoveredRef.current ? 0.015 : 0.025;
            orbitSpeedRef.current += (target - orbitSpeedRef.current) * lerpFactor;

            if (orbitSpeedRef.current > 0.001) {
                orbitAngleRef.current += (0.005 * dt) * orbitSpeedRef.current;
                const wrapper = document.getElementById('orbit-system');
                if (wrapper) {
                    wrapper.style.setProperty('--global-angle', orbitAngleRef.current);
                }
            }
    ...
    const tabMapList = activeSpace === 'prywatne' ? privateTabs : (activeSpace === 'praca' ? workTabs : ghostTabs);
    ...
    const handleCloseTab = (id) => {
        const closeWithAnim = (list, setList) => {
        ...
        if (activeSpace === 'prywatne') closeWithAnim(privateTabs, setPrivateTabs);
        if (activeSpace === 'praca') closeWithAnim(workTabs, setWorkTabs);
        if (activeSpace === 'ghost') closeWithAnim(ghostTabs, setGhostTabs);
    };
        ...
        <>
            ...
            {activeSpace === 'ghost' ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <div className={`rounded-full bg-red-500/10 flex items-center justify-center border border-red-500/30 transition-transform duration-300 ${isActive ? 'w-24 h-24 scale-110' : 'w-12 h-12 group-hover/card:scale-110'}`}>
                        <WifiOff size={isActive ? 40 : 20} className="text-red-400" />
                    </div>
                </div>
            ) : (
                <div className="absolute inset-0 flex items-center justify-center">
    ...
    const renderOrbitTrack = (tabs, radiusClamp, speedMod, delayOffset = 0) => {
        ...
        return (
            <div className="absolute top-1/2 left-1/2 w-0 h-0 z-20 pointer-events-none" style={{ transformStyle: 'preserve-3d', transform: `rotateX(55deg) rotateZ(calc(var(--global-angle) * ${speedMod} * 1deg))` }}>
                {tabs.map((tab, i) => {
                    ...
                    return (
                        <React.Fragment key={tab.id}>
                            <div className="absolute bottom-0 left-0 w-px bg-gradient-to-t from-accent/40 to-transparent origin-bottom opacity-30 animate-cinematic-fade"
                                style={{ height: radiusClamp, transform: `rotateZ(${angleDeg}deg) translateX(-0.5px)`, animationDelay: `${i * 0.1}s` }}></div>
                            ...
                            <div className="absolute top-0 left-0 w-0 h-0 pointer-events-none" style={{ transformStyle: 'preserve-3d', transform: `rotateZ(${angleDeg}deg) translateY(calc(-1 * ${radiusClamp}))` }}>
                                <div className="absolute top-0 left-0 w-0 h-0 pointer-events-none" style={{ transformStyle: 'preserve-3d', transform: `rotateZ(calc(var(--global-angle) * -${speedMod} * 1deg))` }}>
                                    <div className="absolute top-0 left-0 w-0 h-0 pointer-events-none animate-cinematic-enter" style={{ transformStyle: 'preserve-3d', transform: `rotateZ(-${angleDeg}deg) rotateX(-55deg) translate3d(${wobbleX}px, ${wobbleY}px, 0)`, animationDelay: `${i * 0.05}s` }}>
                                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 group/card cursor-pointer animate-organic pointer-events-auto"
                                            style={{ animationDelay: `${i * 1.8}s` }}
                                            onMouseEnter={() => { isOrbitHoveredRef.current = true; }}
                                            onMouseLeave={() => { isOrbitHoveredRef.current = false; }}
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                const setList = activeSpace === 'prywatne' ? setPrivateTabs : (activeSpace === 'praca' ? setWorkTabs : setGhostTabs);
    ...
    return (
        <div className={`fixed inset-0 z-[80000] flex flex-col bg-[#050508]/85 backdrop-blur-3xl overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${isTabMapClosing ? 'opacity-0 scale-[1.02]' : 'opacity-100 scale-100'}`} onClick={closeTabMap}>
  ...
  @keyframes cinematicEnter {
      0% { opacity: 0; filter: blur(20px); transform: rotateZ(var(--angle)) rotateX(-55deg) translate3d(var(--wobbleX), var(--wobbleY), 100px) scale(0.3); }
      100% { opacity: 1; filter: blur(0px); transform: rotateZ(var(--angle)) rotateX(-55deg) translate3d(var(--wobbleX), var(--wobbleY), 0) scale(1); }
  }
  ...
  .animate-cinematic-enter { animation: cinematicEnter 1.2s cubic-bezier(0.22, 1, 0.36, 1) both; }
  .animate-cinematic-fade { animation: cinematicFade 1.5s ease-in-out both; }
`}</style>
            ...
            <div className="absolute top-0 left-0 right-0 px-8 md:px-16 py-8 md:py-12 flex justify-between items-start z-[90000] pointer-events-none animate-slide-down">
                <div>
                    ...
                    <p className="text-xs md:text-sm text-white/50 font-bold uppercase tracking-widest flex items-center gap-3 drop-shadow-md">
                        {activeSpace === 'prywatne' ? <User size={16} className="text-accent" /> : activeSpace === 'praca' ? <Layers size={16} className="text-accent" /> : <Ghost size={16} className="text-[#a855f7]" />}
                        {activeSpace === 'prywatne' ? 'Personal Space' : activeSpace === 'praca' ? 'Work Space' : 'Ghost Session'}
                    </p>
                </div>
                <button onClick={closeTabMap} className="w-12 h-12 rounded-full bg-white/10 backdrop-blur-md hover:bg-white/20 flex items-center justify-center text-white transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] hover:scale-110 shadow-[0_10px_30px_rgba(0,0,0,0.5)] pointer-events-auto border border-white/10"><X size={20} /></button>
            </div>
            ...
            <div id="orbit-system" className="relative w-full h-full animate-pop-in pointer-events-none" style={{ perspective: '1200px', transformStyle: 'preserve-3d' }}>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none z-0 flex items-center justify-center" style={{ transform: 'rotateX(55deg)' }}>
                    <div className="w-[80vw] h-[80vw] max-w-[1200px] max-h-[1200px] rounded-full border border-white/5 opacity-30 animate-cinematic-fade"></div>
                    <div className="absolute w-[50vw] h-[50vw] max-w-[700px] max-h-[700px] rounded-full border border-white/5 border-dashed opacity-40 animate-cinematic-fade"></div>
                    <div className="absolute w-[30vw] h-[30vw] max-w-[400px] max-h-[400px] rounded-full border border-accent-20 opacity-30 animate-ping" style={{ animationDuration: '6s' }}></div>
                </div>
                ...
                {tabMapActive && (
                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[60000] cursor-pointer group animate-organic pointer-events-auto animate-cinematic-enter"
                        style={{ animationDelay: '0s' }}
                        onMouseEnter={() => { isOrbitHoveredRef.current = true; }}
                        onMouseLeave={() => { isOrbitHoveredRef.current = false; }}
                        onClick={(e) => { e.stopPropagation(); closeTabMap(); }}
                    >
