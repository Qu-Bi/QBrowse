...
const searchRegex = /  ipcMain\.on\('set-adblock', \(event, active\) => \{[\s\S]*?if \(BrowserWindow\.getAllWindows\(\)\.length === 0\) \{[\s\S]*?createWindow\(\);[\s\S]*?\}[\s\S]*?\}\);/;
...
const replace = `  ipcMain.on('set-adblock', (event, active) => {
  ...
  ElectronBlocker.fromPrebuiltAdsAndTracking(fetch).then((blocker) => {
    ...
    app.on('web-contents-created', (event, contents) => {
        ...
        // Add YouTube ad speedup logic here carefully
...
if (searchRegex.test(text)) {
    text = text.replace(searchRegex, replace);
